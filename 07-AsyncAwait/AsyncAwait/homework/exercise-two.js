...
const { cyanBright, redBright, yellowBright } = require('./utils');
...
async function problemA() {
  ...
  var poems = await Promise.all(files.map(poem => promisifiedReadFile(poem)));
  yellowBright('-- A. Async version --');
  for (const poem of poems) {
    blue(poem);
  }
  redBright('-- A. Async version done --');
}
...
async function problemB() {
  ...
  var poems = await Promise.all(filenames.map(poem => promisifiedReadFile(poem)));
  yellowBright('-- B. callback version --');
  for (const poem of poems) {
    blue(poem);
  }
  redBright('-- B. callback version done --');
}
...
async function problemC() {
  ...
  yellowBright('-- C. Async version --');
  for (const file of filenames) {
    var stanza = await promisifiedReadFile(file);
    blue(stanza);
  }
...
async function problemD() {
  ...
  redBright('-- D. Async version done --');
