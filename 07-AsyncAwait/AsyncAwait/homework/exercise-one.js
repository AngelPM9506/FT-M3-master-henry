...
const { cyanBright, redBright } = require('./utils');
...
async function problemA() {
  ...
  // AsyncAwait version
  var stanza = await promisifiedReadFile('poem-one/stanza-01.txt');
  cyanBright('-- A. Async version --');
  blue(stanza);
}
...
async function problemB() {
  ...
  // AsyncAwait version
  var stanza2 = await promisifiedReadFile('poem-one/stanza-02.txt');
  cyanBright('-- B. Async version (stanza two) --');
  blue(stanza2);
  var stanza3 = await promisifiedReadFile('poem-one/stanza-03.txt');
  cyanBright('-- B. Async version (stanza three) --');
  blue(stanza3);
}
...
async function problemC() {
  ...
  var stanza2 = await promisifiedReadFile('poem-one/stanza-02.txt');
  cyanBright('-- c. Async version (stanza two) --');
  blue(stanza2);
  var stanza3 = await promisifiedReadFile('poem-one/stanza-03.txt');
  cyanBright('-- c. Async version (stanza three) --');
  blue(stanza3);
  redBright('-- C. Async version done --');
}
...
async function problemD() {
  ...
  cyanBright('-- D. Async version (stanza four) --');
  try {
...
async function problemE() {
  ...
  redBright('-- E. Async version --');
  try {
    var stanza3 = await promisifiedReadFile('poem-one/stanza-03.txt')
    cyanBright('-- E. Async version (stanza three) --');
    blue(stanza3);
    var stanza4 = await promisifiedReadFile('poem-one/wrong-file-name.txt')
    cyanBright('-- E. Async version (stanza four) --');
    blue(stanza4);
  } catch (error) {
    magenta(error);
  }
...
async function problemF() {
  ...
  redBright('-- D. Async version --');
  try {
    var stanza3 = await promisifiedReadFile('poem-one/stanza-03.txt')
    cyanBright('-- D. Async version (stanza three) --');
    blue(stanza3);
    var stanza4 = await promisifiedReadFile('poem-one/wrong-file-name.txt')
    cyanBright('-- D. Async version (stanza four) --');
    blue(stanza4);
  ...
  redBright('-- F. callback version done --');
}
